...
func TestApplyMessage_Private_whenCreatePartyProtectionC1WithPrivacyEnhancementsDisabledAndSPUnsupportedReturnsError(t *testing.T) {
	originalP := private.P
	defer func() { private.P = originalP }()
	mockPM := newMockPrivateTransactionManager()
	private.P = mockPM
	...
	evm := newEVM(cfg)
	evm.ChainConfig().PrivacyEnhancementsBlock = nil
	evm.ChainConfig().Transitions = []params.Transition{
...
func TestApplyMessage_Private_whenCreatePartyProtectionC1WithPrivacyEnhancementsDisabledWhileStandardPrivacyIsSupported(t *testing.T) {
	originalP := private.P
	defer func() { private.P = originalP }()
	mockPM := newMockPrivateTransactionManager()
	private.P = mockPM
	...
	_, err := ApplyMessage(evm, privateMsg, gp)

	assert.True(err == nil, "Transaction passed as expected")
	// check that there is no privacy metadata for the newly created contract
	assert.Len(evm.CreatedContracts(), 1, "Contracts was created")
	mockPM.Verify(assert)
}
